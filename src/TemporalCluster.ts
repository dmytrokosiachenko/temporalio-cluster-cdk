...
import { AuroraServerlessTemporalDatastore, IAuroraServerlessTemporalDatastoreProps, ITemporalDatastore } from './TemporalDatastore';
...
import { FrontendService, HistoryService, MatchingService, SingleService, WebService, WorkerService } from './services/ServerServices';
...
export class TemporalCluster extends Construct implements IConnectable {
  ...
  // FIXME: Refactor this
  private resolveServiceProps(services: ITemporalClusterProps['services']): Required<Omit<ITemporalClusterProps['services'], 'default'>> {
  ...
  private wireUpNetworkAuthorizations(databases: { main: TemporalDatabase; visibility: TemporalDatabase }) {
    const asConnections = (defaultPort: number | undefined, nodeTypes: TemporalNodeType[]) => {
      return new Connections({
        securityGroups: nodeTypes.flatMap(nodeType => this.services[nodeType]?.connections?.securityGroups || []),
