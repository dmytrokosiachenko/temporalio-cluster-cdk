...
export class TemporalNamespaceProvider extends Construct {
  ...
  public static getOrCreate(cluster: TemporalCluster) {
    ...
    const provider = (cluster.node.tryFindChild(id) as TemporalNamespaceProvider) ?? new TemporalNamespaceProvider(cluster, id);
  ...
  private expandPrivilegesToTemporal(cluster: TemporalCluster) {
    this.lambdaFunction.connections.allowToDefaultPort(cluster);
  }
}
